...
export default class Gomoku {
  ...
  static async excuteMove(match: IMatch, movement: { player_id: string; point: { x: number, y: number, color: string } }) {
    ...
    const { player_id, point } = movement;
    let gameover = false;
    // 需要深克隆,不然修改位置部分失败
    ...
    curr_state.board[`${point.x}|${point.y}`] = point.color;
  ...
  static checkWin(state: any, point: { x: number, y: number, color: string }): boolean {
    ...
    for (const [dir1, dir2] of directions) {
      ...
      // 方向1
      for (let m = 1; m <= 4; m++) {
        const x2 = point.x + dir1[0] * m;
        const y2 = point.y + dir1[1] * m;
        if (state.board[`${x2}|${y2}`] !== point.color) {
          break;
        }
        count++;
      }
      // 方向2
      for (let n = 1; n <= 4; n++) {
        const x2 = point.x + dir2[0] * n;
        const y2 = point.y + dir2[1] * n;
        if (state.board[`${x2}|${y2}`] !== point.color) {
          break;
        }
        count++;
